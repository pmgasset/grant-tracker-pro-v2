// Cloudflare Worker for Grant Data Persistence (JavaScript version)
...
async function saveGrants(request, env, corsHeaders) {
...
async function loadGrants(request, env, corsHeaders) {
...
async function updateGrants(request, env, corsHeaders) {
  ...
  try {
    // Load existing data
    const existingData = await env.GRANTS_KV.get(`grants:${userId}`, 'json');
...
async function deleteGrants(request, env, corsHeaders) {
  const url = new URL(request.url);
...
async function saveToD1Database(db, data) {
